...
callTextToDataFrame = function(callText){
  ...
  out = tryCatch({
    ...
    execs = sub('Jr\\.|II|III|IV|V','',execs)
    
    execs = stringr::str_extract(execs, "(\\w+) ([A-Z]\\.[A-Z]\\.) (\\w+)|^(\\w+) (\\w+\\. ){1,5}(\\w+)|(\\w+) (\\w+\\.){1,5}( \\w+)$|(\\w+) (\\w+)")

    execs = unique(append(execs,sub('(\\w+) ([A-Z]\\.[A-Z]\\.) (\\w+)|^(\\w+) (\\w+\\. ){1,5}(\\w+)|(\\w+) (\\w+\\.){1,5}( \\w+)$|(\\w+) (\\w+)', execs)))
    ...
    if(length(analystTest) != 0){
      ...
      analysts = unique(append(analysts,sub('^(\\w+) (\\w+\\. ){1,5}(\\w+)|(\\w+) (\\w+\\.){1,5}( \\w+)$', '\\1 \\3', analysts)))
